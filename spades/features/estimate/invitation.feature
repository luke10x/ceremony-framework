Feature: Engineers can join by invitation

  Alice is Agile Coach
  Bob and Charlie are Engineers

  Scenario: Engineers can join with invite links
    Given Alice created a Ceremony
    And Alice shared an Invite-Link to Bob
    
    When Bob joins a valid Invite-Link
    
    Then Bob should to see the Ceremony

  Scenario: Everybody can see when an Engineer joins
    Given Alice created a Ceremony
    And Bob has joined the Ceremony
    And Alice shared an Invite-Link to Charlie
    
    When Charlie joins a valid Invite-Link
